...
//		cout << "Start " << name << endl;
        ...
        for(int i = 1;i < 32;i++){
            riscvRef.regs[i] = top->VexRiscv->MulAccRegFilePlugin_regFile[i];
        }
		resetDone = true;
	...
	virtual void checks(){
		if(top->VexRiscv->lastStageIsFiring && top->VexRiscv->lastStageInstruction == 0x00000013){
			...
			if(instruction == 0x00000073){
				uint32_t code = top->VexRiscv->MulAccRegFilePlugin_regFile[28];
				uint32_t code2 = top->VexRiscv->MulAccRegFilePlugin_regFile[3];
...
//			#endif
		...
		#ifdef CUSTOM_MUL_ACC
			redo(REDO,WorkspaceRegression("mul_acc").loadHex("../custom/mul_acc/build/mul_acc.hex")->bootAt(0x00000000u)->run(50e3););
		#endif
		
		#ifdef SIMD_MUL_ACC
			redo(REDO,WorkspaceRegression("simd_mul_acc").loadHex("../custom/simd_mul_acc/build/simd_mul_acc.hex")->bootAt(0x00000000u)->run(50e3););
		#endif
		
		#ifdef KERNEL_MUL_ACC
			redo(REDO,WorkspaceRegression("kernel_mul_acc").loadHex("../custom/kernel_mul_acc/build/kernel_mul_acc.hex")->bootAt(0x00000000u)->run(50e3););
		#endif
		
		#ifdef IN_PLACE_ADD
			redo(REDO,WorkspaceRegression("in_place_add").loadHex("../custom/in_place_add/build/in_place_add.hex")->bootAt(0x00000000u)->run(50e3););
		#endif
		
		#ifdef CUSTOM_REG_FILE
			redo(REDO,WorkspaceRegression("custom_reg_file").loadHex("../custom/custom_reg_file/build/custom_reg_file.hex")->bootAt(0x00000000u)->run(50e3););
		#endif
